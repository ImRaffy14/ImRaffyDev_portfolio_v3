...
export const defaultClickableTargetSelector = [
  'a[href]',
  'button:not(:disabled)',
...
export default function TargetCursor({
  targetSelector = defaultClickableTargetSelector,
  spinDuration = 2,
  ...
  const enabled = !isMobile && !reducedMotion && !stripUiMotion
  ...
  useEffect(() => {
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      corners.forEach((corner, i) => {
        const p = targetCornerPositionsRef.current![i]
        gsap.to(corner, {
          x: p.x - cursorX,
          y: p.y - cursorY,
          duration: 0.2,
